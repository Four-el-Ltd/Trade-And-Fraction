...
public class BedrockWand extends Item {
    ...
    public boolean onItemUse(final ItemStack stack, final EntityPlayer player, final World world, final int x,
        final int y, final int z, final int dir, final float mx, final float my, final float mz) {
        ...
        if (target != null && world.getBlock(x, y, z) instanceof BlockBedrockOreTE && !player.isSneaking()) {
            final BlockBedrockOreTE.TileEntityBedrockOre teb = (BlockBedrockOreTE.TileEntityBedrockOre) world
                .getTileEntity(x, y, z);
        ...
        if (player.isSneaking()) {
            if (player.inventory.getStackInSlot(0) != null) {
                ...
                stack.stackTagCompound.setInteger(
                    ...
                    player.inventory.getStackInSlot(0)
                        .getItemDamage());
                if (player.inventory.getStackInSlot(1)
                    .getItem() instanceof ItemFluidIdentifier) {
                    stack.stackTagCompound.setInteger(
                        "fluid_type",
                        ItemFluidIdentifier.getType(player.inventory.getStackInSlot(1))
                            .getID());
                ...
                if (world.isRemote) {
                    player.addChatMessage(
                        new ChatComponentText(
                            "Тип руды установлен на" + Item.getItemById(stack.stackTagCompound.getInteger("id_target"))
                                .getUnlocalizedName()));
                }
            }
        }
